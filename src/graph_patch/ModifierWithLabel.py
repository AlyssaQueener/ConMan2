...
class ModifierWithLabel:
    ...
    def calculate_modifications_for_geo_node(self, node_init, node_updt, semantic_patch_pattern, label):
        ...
        if has_changes:
            ...
            if entity_node:
                ...
                setattr(entity_node, "label", label)
                if not hasattr(entity_node, "label_info"):
                    setattr(entity_node, "label_info", "Geometry")
                entity_node.save()
            setattr(node_init, "change_type", "modified")
            node_init.save()
            
    def calculate_modifications_for_geo_node_room_size(self, node_init, node_updt, semantic_patch_pattern, label):
        geo_node_entity = node_init.EntityType
        ...
        if has_changes:
            entity_node = node_init.relation_geo.all()[0]
            if entity_node and entity_node.EntityType == "IfcSpace":
                setattr(entity_node, "geo_modification", 1.0)
                setattr(entity_node, "change_type", "modified")
                setattr(entity_node, "label", label)
                if not hasattr(entity_node, "label_info"):
    ...
    def semantic_patch(self, equivalent_nodes_init, semantic_patch, label):
        """
                ...
                if property_key not in ["timestamp", "element_id_property", "p21_id"]:
                    ...
                    if property_value != property_value_updt:
                        ...
                        if property_key == "materials":
                            ...
                            setattr(node_init, "label_info_material", "Material")
    ...
    def modify_semantic(self, project_id: str, timestamp_init:str, timestamp_updt:str, label:str):
        """
        ...
        self.semantic_patch(equivalent_nodes_init, semantic_patch_pattern,label)
        for node_init in equivalent_geo_nodes_init:
            # Find the corresponding equivalent node in the db.
            node_updt = node_init.equivalent_to.all()[0]
            self.calculate_modifications_for_geo_node(node_init, node_updt, semantic_patch_pattern,label)
    ...
    def modify_semantic_room_size(self, project_id: str, timestamp_init:str, timestamp_updt:str, label:str):
        """
        Use the two models that have been diffed and create a semanicpatch.
        """
        semantic_patch_pattern= []
        ...
        for node_init in equivalent_geo_nodes_init:
            # Find the corresponding equivalent node in the db.
            node_updt = node_init.equivalent_to.all()[0]
            self.calculate_modifications_for_geo_node_room_size(node_init, node_updt, semantic_patch_pattern,label)
        with open(f"patch_data/Patch_Sema_{project_id}_{timestamp_init}_{timestamp_updt}.json", "w") as f:
            json.dump(semantic_patch_pattern, f, indent=4)
        path_semantic = f"patch_data/Patch_Sema_{project_id}_{timestamp_init}_{timestamp_updt}.json"
        return path_semantic
